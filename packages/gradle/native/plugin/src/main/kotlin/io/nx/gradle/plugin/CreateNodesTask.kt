...
abstract class CreateNodesTask : DefaultTask() {
    ...
    fun processTargetsForProject(
        ...
        project.getTasks().forEach { task ->
            ...
            target.put("command", taskCommand + task.name)
            ...
            if (task.name.startsWith("compileTest")) {
                addTestCiTarget(inputs, taskCommand, target, targets, targetGroups, projectRoot, workspaceRoot)
            }
        }
    ...
    fun addTestCiTarget(
        ...
        testFiles.filter { testFile ->
            ...
            val metadata = mutableMapOf<String, Any?>()
            metadata.put("description", "Runs Gradle test ${fileName} in CI")
            metadata.put("technologies", arrayOf("Gradle"))
            val testCiTarget = target.toMutableMap()
            testCiTarget.put("command", "${taskCommand}test --tests ${fileName}")
        ...
        if (!dependsOn.isEmpty()) {
            val testCiTarget = target.toMutableMap()
            val metadata = mutableMapOf<String, Any?>()
            metadata.put("description", "Runs Gradle Tests in CI")
            metadata.put("technologies", arrayOf("Gradle"))
